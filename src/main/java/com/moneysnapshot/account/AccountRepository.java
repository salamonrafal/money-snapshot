...
public interface AccountRepository extends JpaRepository<Account, UUID> {
    ...
    @Query("select account from Account account join fetch account.bank join fetch account.owner where account.id = :id")
    ...
    @Query("select account from Account account join fetch account.bank join fetch account.owner order by account.name")
    List<Account> findAllWithBankOrderByName();

    @Query("select account from Account account join fetch account.bank join fetch account.owner where account.owner.id = :ownerId order by account.name")
