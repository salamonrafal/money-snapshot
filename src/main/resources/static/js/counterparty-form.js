window.MoneySnapshotCounterpartyForm = (() => {
    function create({root = document, initialId = null, onSuccess = null} = {}) {
        let counterpartyId = initialId;
        const counterpartyForm = root.querySelector("#counterparty-form");
        const counterpartyPageTitle = root.querySelector("#counterparty-form-page-title");
        const counterpartyPageSubtitle = root.querySelector("#counterparty-form-page-subtitle");
        const counterpartyFormMessageContainer = root.querySelector("#counterparty-form-message-container");
        const counterpartyFormMessage = root.querySelector("#counterparty-form-message");
        const counterpartyNameInput = root.querySelector("#counterparty-name");
        const counterpartyBankAccountNumberInput = root.querySelector("#counterparty-bank-account-number");
        const counterpartyBankAccountBankInfo = root.querySelector("#counterparty-bank-account-bank-info");
        const counterpartyAddressInput = root.querySelector("#counterparty-address");
        const counterpartyNoteInput = root.querySelector("#counterparty-note");
        const saveCounterpartyButton = root.querySelector("#save-counterparty");
        ...
        function handleCounterpartyFormLanguageChange(nextMessages) {
            ...
            if (!onSuccess) document.title = `${title} | ${counterpartyFormMessages["app.name"]}`;
        ...
        return {
            isSaving: () => saving,
            setMessages: handleCounterpartyFormLanguageChange,
            load: loadCounterparty,
            showError: (error) => setCounterpartyFormMessage(error.message, "error"),
            prepare(counterparty = null) {
                ...
                setCounterpartyBankInfo("");
                saveCounterpartyButton.disabled = false;
                if (counterparty) fillCounterpartyForm(counterparty);
                handleCounterpartyFormLanguageChange(counterpartyFormMessages);
            }
        };
    }
    return {create};
...
if (!document.querySelector("#counterparty-editor-modal")) {
    const controller = window.MoneySnapshotCounterpartyForm.create({initialId: document.body.dataset.counterpartyId || null});
    MoneySnapshotI18n.init({
        endpoint: "/api/counterparty-form/messages",
        onLanguageChange: ({messages}) => controller.setMessages(messages)
    }).then(() => controller.load()).catch((error) => controller.showError(error));
}
